...
@section('content')
    <div class="user-profile-body__wrapper">
        <div class="profile-item-wrapper">
            <div class="card-item__inner m-0">
                <form action="{{ route('ticket.store') }}" class="disableSubmission" method="post"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 select2-parent">
                            <div class="form-group">
                                <label class="form--label">@lang('Priority')</label>
                                <select name="priority" class="form--control select2" data-minimum-results-for-search="-1"
                                    required>
                                    <option value="{{ Status::PRIORITY_HIGH }}" @selected(old('priority') == Status::PRIORITY_HIGH)>
                                        @lang('High')</option>
                                    <option value="{{ Status::PRIORITY_MEDIUM }}" @selected(old('priority') == Status::PRIORITY_MEDIUM)>
                                        @lang('Medium')</option>
                                    <option value="{{ Status::PRIORITY_LOW }}" @selected(old('priority') == Status::PRIORITY_LOW)>
                                        @lang('Low')</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-9">
                            <button type="button" class="btn btn-dark addAttachment mb-2">
                            ...
                            <div class="row fileUploadsContainer gy-2 mb-2"></div>
                        </div>
                        <div class="col-md-3">
