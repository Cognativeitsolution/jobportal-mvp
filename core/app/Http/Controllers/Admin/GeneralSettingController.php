...
class GeneralSettingController extends Controller {
    ...
    public function systemConfigurationSubmit(Request $request) {
        ...
        $general->free_job_post         = $request->free_job_post ? Status::ENABLE : Status::DISABLE;
        $general->job_post_payment      = $request->job_post_payment ? Status::ENABLE : Status::DISABLE;
        $general->save();
    ...
    public function profileUpdateSettingSubmit(Request $request) {
        ...
        $sum = collect($request->resume_percentage)->sum();
        if ($sum != 100) {
            $notify[] = ['error', 'The total percentage must be equal to 100'];
            return back()->withNotify($notify);
        }

        $general                    = gs();
        $general->resume_percentage = $request->resume_percentage;
        $general->save();

        $notify[] = ['success', 'Resume update settings updated successfully'];
        return back()->withNotify($notify);
    }
}
